    ...
    try:
        ...
        for p in soup.find_all('p'):
            ...
            if 'Redakteure:' in text or '=' in text:
                ...
                # Name ends at a comma or end of string. A trailing sentence period
                # (only on the final entry, e.g. "tr = Thomas Röder.") is stripped
                # below, but abbreviation periods ("jun.") are kept because those
                # entries are followed by a comma, not end-of-string.
                pattern = r'([a-z]{2})\s*=\s*([^(,]+?)(?:\s*\([^)]*\))?\s*(?:,|$)'
                for m in re.finditer(pattern, text, re.IGNORECASE):
                    code = m.group(1).lower().strip()
                    name = m.group(2).strip()
                    # Only the match that runs to the end of the paragraph can have
                    # a sentence-terminating period; strip a single trailing one.
                    if m.end() >= len(text.rstrip()) and name.endswith('.'):
                        name = name[:-1].strip()
