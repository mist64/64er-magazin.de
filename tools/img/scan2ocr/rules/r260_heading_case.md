...
handled by rule 200 (Leserforum's import-time ALL CAPS → Title Case
...
Cross-reference: this mirrors rule 290's `DEFAULT: DON'T CHANGE HEADER
LEVELS` framing. Rule 290 protects heading *levels*; this rule
...
6. **Rename the article HTML file to match the new h1.** Filenames
   ...
   page-number convention from rule 080 still applies (`<startpage> <h1>.html`).
7. **Update the `<title>` tag too.** Rule 080 emits `<title>{h1_plain}</title>`
   from the same h1 text. When rule 260 rewrites the h1, the
...
- A heading that contains an OCR artifact like
  ...
  autolink stripped first (rule 270), then the heading case
  converted — running rule 270 before rule 260 avoids touching the
  same h1 twice.
