...
CMYK_TOOL="${SCRIPT_DIR}/cmyk_reconstruction/target/release/cmyk_reconstruction"

# Build the Rust tool if needed
if [ ! -x "$CMYK_TOOL" ]; then
  echo "Building CMYK reconstruction tool..."
  (cd "${SCRIPT_DIR}/cmyk_reconstruction" && cargo build --release)
fi
...
for in_filename in ???.png; do
	...
	out_filename_600="${out_600}/${id}_600.png"
	out_filename_150="${out_150}/${id}_150.png"
	out_filename_150_cropped="${out_cropped}/${id}_150_cropped.png"
	...
	# convert to CMYK using Rust tool (includes level stretching)
	"${CMYK_TOOL}" ${in_filename} ${tmp1}
