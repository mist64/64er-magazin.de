...
visible exactly where it matters -- where the window overhangs the known page.
  ...
  green   window from this page's OWN detected logo
  orange  window from an INTERPOLATED anchor (no logo on this page) -- a different colour
          because it is inferred, and must not read as measured
  magenta the anchor itself
"""
...
def one(item):
    ...
    col = COL_LOGO if w["src"] == "logo" else COL_INTERP
    x0, y0 = w["x0"], w["y0"]
    x1, y1 = x0 + w["w"], y0 + w["h"]
